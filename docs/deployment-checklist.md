...
1. **Database**: create Turso DB, set `DATABASE_URL` + `DATABASE_AUTH_TOKEN` with **builds** scope. Migrations apply during each deploy (the root `netlify.toml` build command migrates before building); verify `[migrate]` lines in the build log — `applied` on the first deploy, `already applied` on later ones. Leave the site's base/package directory unset — the root `netlify.toml` overrides UI build settings.
...
7. **Rollback**: see docs/deployment.md — per-route redirects, provider kill-switches via env vars; to stop build-time migrations, remove the `run migrate` step from the root `netlify.toml`'s build command; migrations are additive-only, so no schema rollback is ever required.
