...
import {
  ...
  type RubygemsRegistryOptions,
} from "../src/registries/rubygems.js";

const clock = { nowMs: () => 1_000 };

function baseOptions(overrides: Partial<RubygemsRegistryOptions> = {}): RubygemsRegistryOptions {
  return {
...
describe("normalizeRubygemsName", () => {
  it("trims but preserves case", () => {
    expect(normalizeRubygemsName("  Rails  ")).toEqual({ ok: true, name: "Rails" });
    expect(normalizeRubygemsName("rack")).toEqual({ ok: true, name: "rack" });
...
describe("createRubygemsRegistry lookup classification", () => {
  it("sends the identifying User-Agent and Accept headers", async () => {
    const fetchImpl = fakeFetch(new Response(JSON.stringify({ name: "Rails" }), { status: 200 }));
    await createRubygemsRegistry(baseOptions({ fetchImpl })).lookup("Rails");
    const headers = headersOf(fetchImpl);
    expect(headers.get("user-agent")).toBe("isittaken/1.2.3 (+https://example.test/repo)");
    expect(headers.get("accept")).toBe("application/json");
  });

  it("classifies 200 with a parseable gem payload as taken", async () => {
    const fetchImpl = fakeFetch(
      new Response(JSON.stringify({ name: "Rails", version: "8.0.0" }), { status: 200 }),
    );
    const result = await createRubygemsRegistry(baseOptions({ fetchImpl })).lookup("Rails");
    expect(result).toEqual({ status: "taken", checkedAtMs: 1_000 });
    expect(fetchImpl).toHaveBeenCalledWith(
      "https://rubygems.test/api/v1/gems/Rails.json",
      expect.anything(),
    );
  });
  ...
  it("classifies the documented 404 not-found as available", async () => {
    ...
    const result = await createRubygemsRegistry(baseOptions({ fetchImpl })).lookup("maybe-free");
  ...
  it("never reports availability for rate limits, timeouts, or errors", async () => {
    ...
    for (const [failure, reason, mode] of cases) {
      ...
      const result = await createRubygemsRegistry(baseOptions({ fetchImpl })).lookup("laser");
  ...
  it("treats non-JSON 200 bodies as ambiguous/unknown", async () => {
    ...
    const result = await createRubygemsRegistry(baseOptions({ fetchImpl: fetchText })).lookup(
      "laser",
    );
    expect(result.status).toBe("unknown");
