...
/**
 * AI shard: the OpenRouter cache family. Daily at 04:23 UTC — a different
 * minute from the language and authentication shards.
 */
...
export async function runOnce(nowMs: number): Promise<void> {
  ...
  try {
    await pruneCacheFamily("openrouter", db, options);
  } catch (error) {
...
export default schedule("23 4 * * *", async () => {
